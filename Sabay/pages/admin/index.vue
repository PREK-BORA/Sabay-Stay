...
const totalRooms = ref<number>(156)
const occupiedRooms = ref<number>(112)
...
const fetchTotalRevenue = async () => {
  ...
  try {
    const q = query(collection(db, 'bookings'), where('status', 'in', ['Paid', 'Confirmed', 'Completed']))
    const querySnapshot = await getDocs(q)
    
    let sum = 0
    querySnapshot.forEach((docSnap) => {
      const data = docSnap.data()
      sum += Number(data.totalPrice || data.amount || data.price || 0)
    })
    
    totalRevenueAmount.value = sum
  } catch (error) {
